...
- **キャラクターシート & 出会いのタイムライン**: 各キャラクターに自由に項目(年齢、種族、所属国家など何でも)を追加・削除でき、人物同士が交わる「出会い」や時点ごとの「変化」も記録します。本質的にリレーショナルなデータなのでgitファイルではなくPostgresに保存し、表形式での編集とキャラクター↔出会いの相互参照を可能にしています。
